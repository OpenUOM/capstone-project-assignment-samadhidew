...
export class TeacherTableComponent implements OnInit {
  ...
  search(value:string) {
    if (value.length <= 0) {
      // If the search input is empty, reset the teacher data to its original state
      this.getTeacherData();
    } else {
      // Filter the teacher data based on the search value
      this.teacherData = this.teacherData.filter((teacher) => {
        // Case-insensitive search by converting both name and search value to lowercase
